...
namespace stream
{
  ...
  namespace reader
  {
    ...
    {
      static StreamReader instance(video);
      return instance;
    }
    ...
    StreamReader::open_input_file()
    {
      ...
      int ret =
        avformat_open_input(&format_ctx_,
                            StreamData::Instance().video_name_get().c_str(),
                            nullptr, nullptr);
      if (ret < 0)
      {
      ...
      if (ret < 0)
      {
        std::cerr << "Cannot find a video stream in the input file"
                  << std::endl;
        std::exit(ret);
      }

      vstream_idx_ = ret;
      StreamData::Instance().nb_frames_set(
        format_ctx_->streams[vstream_idx_]->nb_frames
      );
      StreamData::Instance().fps_set(
        av_q2d(format_ctx_->streams[vstream_idx_]->r_frame_rate)
      );
      codec_ctx_ = format_ctx_->streams[vstream_idx_]->codec;
      StreamData::Instance().width_set(codec_ctx_->coded_width);
      StreamData::Instance().height_set(codec_ctx_->coded_height);
